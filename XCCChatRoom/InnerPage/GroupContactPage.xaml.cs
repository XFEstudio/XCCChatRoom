...
public partial class GroupContactPage : ContentPage
{
                ...
                if (UserInfo.IsLoginSuccessful)
                {
                    ...
                    if (string.IsNullOrWhiteSpace(targetGroupName))
                    {
                        if (targetGroupName == string.Empty)
                            await DisplayAlert("错误", "群组名称不能为空", "确定");
                        return;
                    }
                    else
                    {
                        var userNameInGroup = await DisplayPromptAsync("群内身份", "请输入在该群中的名称", "确定", null, "群内名称", 20, Keyboard.Default, UserInfo.StaticUserName);
                        if (string.IsNullOrWhiteSpace(userNameInGroup))
                        {
                            if (userNameInGroup == string.Empty)
                                await DisplayAlert("错误", "群内名称不能为空", "确定");
                            return;
                        }
                        try
                        {
                        ...
                        {
                            await DisplayAlert("群名重复", "群组名称不能重复\n原异常：" + ex.Message, "确定");
                        }
                    }
                }
                else
                {
                    await DisplayAlert("未登录", "请先登录", "确定");
                }
            })
        });
    ...
    private async void GroupCardView_Swipe(object sender, GroupCardViewSwipeEventArgs e)
    {
        ...
        var result = await DisplayAlert("删除", "是否删除该群组？", "是", "否");
        if (result)
        {
