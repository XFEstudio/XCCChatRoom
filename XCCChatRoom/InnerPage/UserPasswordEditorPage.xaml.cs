...
public partial class UserPasswordEditorPage : ContentPage
{
    ...
    private async void OldPassword_TextChanged(object sender, FocusEventArgs e)
    {
    ...
    private async void NewPassword_TextChanged(object sender, FocusEventArgs e)
    {
        if (NewPassword.Text.PasswordEditor()) { flag2 = true; }
        else { await DisplayAlert("出错了", "密码不符合要求", "确定"); }
    }

    private async void NewPasswordConfirmation_TextChanged(object sender, FocusEventArgs e)
    {
        if(NewPassword.Text == NewPasswordConfirmation.Text) { flag3 = true; }
        else { await DisplayAlert("出错了", "新旧密码不一致", "确定"); }
    }
